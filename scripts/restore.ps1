...
function Test-Backup-Has-External-Db([string] $BackupName) {
    $result = docker run --rm -v $BackupName`:/backup ubuntu bash -c "
    cd /backup &&
    if [ -f $DbDataArchiveName ]; then
        echo 1
    else
        echo 0
    fi"
    # If the volume is missing the db archive file, then it was likely created with the external db flag
    $result -eq 0
}
...
$UsingExternalDb = Test-Backup-Has-External-Db $BackupVolumeName
